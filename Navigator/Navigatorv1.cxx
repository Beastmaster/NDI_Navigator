...
Navigator::Navigator() : m_StateMachine(this)
{
 ...
 //complete table for state: LoadingImage State
  ...
  {
      MRIOverlayRepresentationType::Pointer r = MRIOverlayRepresentationType::New();  //Sun adds
      m_MRIOverlayRepresentation.push_back( r );
  }
/*
  std::ofstream ofile;
...
//  m_ViewerGroup->RequestUpdateOverlays(); //New adds
  ...
  if ( m_ImageObserver.IsNotNull() )
  {
    ...
    this->ConnectImageRepresentation();//Sun adds
    this->ReadFiducials();    
    this->RequestChangeSelectedFiducial();
  ...
  //  this->RequestChangeSelectedFiducial();
   ...
   for( int i=0; i<3; i++)
      ...
      if (m_ImageSpatialObject1)
        {
        m_MRIOverlayRepresentation[i]->SetBGOpacity(0.1); // This should be called after added to view
  //      m_CTPETImageRepresentation[i]->SetFGImageLUT(0.5);
        }
      else
        {
        m_MRIOverlayRepresentation[i]->SetBGOpacity(0.0);
        }
      }
	  ...
	  vtkCamera * camera;
      m_ViewerGroup->m_AxialView->RequestResetCamera();
      camera = m_MRIOverlayRepresentation[0]->RequestReslice();
      m_ViewerGroup->m_AxialView->RequestSetActiveCamera( camera );
      m_ViewerGroup->m_AxialView->SetCameraZoomFactor(2.5);

	  m_ViewerGroup->m_SagittalView->RequestResetCamera();
      camera = m_MRIOverlayRepresentation[1]->RequestReslice();
      m_ViewerGroup->m_SagittalView->RequestSetActiveCamera( camera );
      m_ViewerGroup->m_SagittalView->SetCameraZoomFactor(2.5);

	  m_ViewerGroup->m_CoronalView->RequestResetCamera();
      camera = m_MRIOverlayRepresentation[2]->RequestReslice();
      m_ViewerGroup->m_CoronalView->RequestSetActiveCamera( camera );
      m_ViewerGroup->m_CoronalView->SetCameraZoomFactor(2.5);
	    // add reslice plane representations to the 3D views
  /**    m_MRIOverlayRepresentation2[0] = m_MRIOverlayRepresentation[0]->Copy();
      m_ViewerGroup->m_3DView->RequestAddObject( m_MRIOverlayRepresentation2[0] );

      m_MRIOverlayRepresentation2[1] = m_MRIOverlayRepresentation[1]->Copy();
      m_ViewerGroup->m_3DView->RequestAddObject( m_MRIOverlayRepresentation2[1] );

      m_MRIOverlayRepresentation2[2] = m_MRIOverlayRepresentation[2]->Copy();
      m_ViewerGroup->m_3DView->RequestAddObject( m_MRIOverlayRepresentation2[0] );**/
...
void Navigator::TrackerRegistrationProcessing()
{
  ...
  if( lrtcb->GotTransform() )
  {
    ...
    if( lRmscb->GotRegistrationError() )
    {
       ...
       m_ViewerGroup->m_AxialViewAnnotation->RequestSetAnnotationText( 1, buf );
       m_ViewerGroup->m_AxialViewAnnotation->RequestSetFontColor(1, 0, 1, 0);
