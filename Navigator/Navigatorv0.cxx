...
Navigator::Navigator() : m_StateMachine(this)
{
 ...
 //complete table for state: LoadingImage State
  ...
  m_MRIRepresentation.clear();
  for (unsigned int i=0; i<4; i++)
    {
    MRIOverlayRepresentationType::Pointer rep = MRIOverlayRepresentationType::New();
    m_MRIRepresentation.push_back( rep );
    }

  /** Create image oblique slice representations  */
  m_MRIOverlayRepresentation.clear();

  for (unsigned int i=0; i<4; i++)
  {
      MRIOverlayRepresentationType::Pointer rep = MRIOverlayRepresentationType::New();  //Sun adds
      m_MRIOverlayRepresentation.push_back( rep );
  }
...
{
  igstkLogMacro2( m_Logger, DEBUG, 
                    "Navigator::LoadAtlasProcessing called...\n" )
 ...
 //  Image = DeepCopy(m_ImageReader);
   ...
   //  fl_beep( FL_BEEP_ERROR );
     ...
     m_ImageObserver1->RemoveAllObservers();
     m_ImageObserver1 = NULL;
...
*---------------------------------------------------------------------
*/
void Navigator::ConnectImageRepresentation(int mri)
{
  ...
  { 
	  for( int i=0; i<3; i++)
      {
      m_MRIRepresentation[i]->RequestSetBGImageSO( m_ImageSpatialObject );
      m_MRIRepresentation[i]->RequestSetFGImageSO( m_FGImageSpatialObject );
      }
  
    m_MRIRepresentation[0]->SetOrientation(
      MRIOverlayRepresentationType::Axial );
    m_MRIRepresentation[1]->SetOrientation(
      MRIOverlayRepresentationType::Sagittal );
    m_MRIRepresentation[2]->SetOrientation(
      MRIOverlayRepresentationType::Coronal );

	m_ViewerGroup->m_AxialView->RequestRemoveObject( m_MRIRepresentation[0] );
	m_ViewerGroup->m_AxialView->RequestRemoveObject( m_MRIOverlayRepresentation[0] );
	m_ViewerGroup->m_AxialView->RequestAddObject(m_MRIRepresentation[0]);
	m_ViewerGroup->m_SagittalView->RequestRemoveObject( m_MRIRepresentation[1] );
	m_ViewerGroup->m_SagittalView->RequestRemoveObject( m_MRIOverlayRepresentation[1] );
	m_ViewerGroup->m_SagittalView->RequestAddObject(m_MRIRepresentation[1]);
    m_ViewerGroup->m_CoronalView->RequestRemoveObject( m_MRIRepresentation[2] );
	m_ViewerGroup->m_CoronalView->RequestRemoveObject( m_MRIOverlayRepresentation[2] );
	m_ViewerGroup->m_CoronalView->RequestAddObject(m_MRIRepresentation[2]);

	for ( int i=0; i<3; i++)
      {
      if (m_FGImageSpatialObject)
        {
        m_MRIRepresentation[i]->SetBGOpacity(0.5); // This should be called after added to view
        }
      else
        {
        m_MRIRepresentation[i]->SetBGOpacity(1.0);
        }
      }
  }
  else
    ...
    for( int i=0; i<6; i++)
      {
      m_MRIOverlayRepresentation[i]->RequestSetBGImageSO( m_ImageSpatialObject );
      m_MRIOverlayRepresentation[i]->RequestSetFGImageSO( m_ImageSpatialObject1 );
      }

    m_MRIOverlayRepresentation[0]->SetOrientation(
      MRIOverlayRepresentationType::Axial );
    m_MRIOverlayRepresentation[1]->SetOrientation(
      MRIOverlayRepresentationType::Sagittal );
    m_MRIOverlayRepresentation[2]->SetOrientation(
      MRIOverlayRepresentationType::Coronal );

	m_MRIOverlayRepresentation[3]->SetOrientation(
      MRIOverlayRepresentationType::Axial );
    m_MRIOverlayRepresentation[4]->SetOrientation(
      MRIOverlayRepresentationType::Sagittal );
    m_MRIOverlayRepresentation[5]->SetOrientation(
      MRIOverlayRepresentationType::Coronal );

	m_ViewerGroup->m_AxialView->RequestRemoveObject( m_AxialPlaneRepresentation );
	m_ViewerGroup->m_AxialView->RequestRemoveObject( m_MRIOverlayRepresentation[0] );
	m_ViewerGroup->m_AxialView->RequestAddObject(m_MRIOverlayRepresentation[0]);
	m_ViewerGroup->m_SagittalView->RequestRemoveObject( m_SagittalPlaneRepresentation );
	m_ViewerGroup->m_SagittalView->RequestRemoveObject( m_MRIOverlayRepresentation[1] );
	m_ViewerGroup->m_SagittalView->RequestAddObject(m_MRIOverlayRepresentation[1]);
    m_ViewerGroup->m_CoronalView->RequestRemoveObject( m_CoronalPlaneRepresentation );
	m_ViewerGroup->m_CoronalView->RequestRemoveObject( m_MRIOverlayRepresentation[2] );
	m_ViewerGroup->m_CoronalView->RequestAddObject(m_MRIOverlayRepresentation[2]);
	//ready add


    for ( int i=0; i<3; i++)
      {
      if (m_ImageSpatialObject1)
        {
        m_MRIOverlayRepresentation[i]->SetBGOpacity(0.5); // This should be called after added to view
        //m_CTPETImageRepresentation[i]->SetFGImageLUT(0.5)
        }
      else
        {
        m_MRIOverlayRepresentation[i]->SetBGOpacity(1.0);
        }
      }
    }
  ...
  for ( int i=0; i<3; i++)
    {
    m_MRIRepresentation[i]->GetExtent(min, max);

    int slice = static_cast< int > ( (min + max) / 2.0 );
    //m_CTCTImageRepresentation[i]->RequestSetSliceNumber( slice );
    m_ViewerGroup->m_Sliders[i]->minimum( min );
    m_ViewerGroup->m_Sliders[i]->maximum( max );
    m_ViewerGroup->m_Sliders[i]->value( slice );
    m_ViewerGroup->m_Sliders[i]->activate();
    }

  index[0] = m_ViewerGroup->m_Sliders[1]->value();
  index[1] = m_ViewerGroup->m_Sliders[2]->value();
  index[2] = m_ViewerGroup->m_Sliders[0]->value();
...
//  m_ViewerGroup->m_CoronalView->SetCameraParallelProjection(true);
  ...
  // reset the cameras in the different views
  m_ViewerGroup->m_AxialView->RequestResetCamera();
  m_ViewerGroup->m_AxialView->SetCameraFocalPoint(m_ImageCenter[0],m_ImageCenter[1],m_ImageCenter[2]);
  m_ViewerGroup->m_AxialView->SetCameraZoomFactor(2.0);

  m_ViewerGroup->m_SagittalView->RequestResetCamera();
  m_ViewerGroup->m_SagittalView->SetCameraFocalPoint(m_ImageCenter[0],m_ImageCenter[1],m_ImageCenter[2]);
  m_ViewerGroup->m_SagittalView->SetCameraZoomFactor(2.0);

  m_ViewerGroup->m_CoronalView->RequestResetCamera();
  m_ViewerGroup->m_CoronalView->SetCameraFocalPoint(m_ImageCenter[0],m_ImageCenter[1],m_ImageCenter[2]);
  m_ViewerGroup->m_CoronalView->SetCameraZoomFactor(2.0);
...
{   
  ...
  for(unsigned int i=0; i<3; i++ )
    {
    m_MRIRepresentation[i]->SetResliceTransform( t );
    m_MRIRepresentation[i]->RequestReslice( );
    m_MRIOverlayRepresentation[i]->SetResliceTransform( t );
